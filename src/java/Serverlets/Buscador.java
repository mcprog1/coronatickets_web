...
import Controller.BuscadorController;
...
public class Buscador extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        BuscadorController controller = new BuscadorController();
        //processRequest(request, response);
         
        String parts[] = request.getRequestURI().split("/coronatickets_web/");
        
        switch (parts[1]) {
            case "buscador/obtenerBusqueda":
                controller.buscador(request,response);
            break;
            default:
                try ( PrintWriter out = response.getWriter()) {
                /* TODO output your page here. You may use following sample code. */
